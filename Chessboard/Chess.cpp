...
Chess::Chess(int lightsPin, int numLights, int onPin, int cheatPin, int colourPin1, int colourPin2, int colourPin3,
  ...
  pixels(numLights, lightsPin, NEO_GRB + NEO_KHZ800)
  {
    ...
    int _onPin = onPin;
    int _cheatPin = cheatPin;

    int _colourPin1 = colourPin1; // the 3 buttons to change colours
    int _colourPin2 = colourPin2;
    int _colourPin3 = colourPin3;




    //Digital Pins
    int _columnZero = columnZero;
    int _columnOne = columnOne;
    int _columnTwo = columnTwo;
    int _columnThree = columnThree;
    int _columnFour = columnFour;
    int _columnFive = columnFive;
    int _columnSix = columnSix;
    int _columnSeven = columnSeven;

    int _rowZero = rowZero;
    int _rowOne = rowOne;
    int _rowTwo = rowTwo;
    int _rowThree = rowThree;
    int _rowFour = rowFour;
    int _rowFive = rowFive;
    int _rowSix = rowSix;
    int _rowSeven = rowSeven;
    ...
    int _testColumnZero = testColumnZero;       //   analogRead(A0)
    int _testColumnOne = testColumnOne;          // analogRead(A1)
    int _testColumnTwo = testColumnTwo;           // analogRead(A2)
    int _testColumnThree = testColumnThree;       // analogRead(A3)
    int _testColumnFour = testColumnFour;        //analogRead(A4)
...
void Chess::initChess(){
  ...
  pinMode(_columnThree, OUTPUT);
  pinMode(_columnFour, OUTPUT);
  pinMode(_columnFive, OUTPUT);
  pinMode(_columnSix, OUTPUT);
  pinMode(_columnSeven, OUTPUT);
  pinMode(_rowZero, OUTPUT);
  pinMode(_rowOne, OUTPUT);
  pinMode(_rowTwo, OUTPUT);
  pinMode(_rowThree, OUTPUT);
  pinMode(_rowFour, OUTPUT);
  pinMode(_rowFive, OUTPUT);
  pinMode(_rowSix, OUTPUT);
  pinMode(_rowSeven, OUTPUT);
  
  pinMode(_testColumnZero, INPUT);    
  pinMode(_testColumnOne, INPUT);
  pinMode(_testColumnTwo, INPUT);
  pinMode(_testColumnThree, INPUT);
  pinMode(_testColumnFour, INPUT);  
  pinMode(_testColumnFive, INPUT);
  pinMode(_testColumnSix, INPUT);  
  pinMode(_testColumnSeven, INPUT);

  pinMode(_colourPin1, INPUT_PULLUP);    
  pinMode(_colourPin2, INPUT_PULLUP);
  pinMode(_colourPin3, INPUT_PULLUP);    
  pinMode(_onPin, INPUT_PULLUP);  
  pinMode(_cheatPin, INPUT_PULLUP); 
    ...
    for (i = 0 ; i<8; i++) { 
      ...
      switch (i) {
        case 0:
          ...
          break;
        case 1:      
          digitalWrite(_rowOne, LOW);
          break;
        case 2:
          digitalWrite(_rowTwo, LOW);
          break;
        case 3:
          digitalWrite(_rowThree, LOW);
          break;
        case 4:
          digitalWrite(_rowFour, LOW);
          break;
        case 5:
          digitalWrite(_rowFive, LOW);
          break;
        case 6:
          digitalWrite(_rowSix, LOW); 
          break;
        case 7: 
...
void Chess::cheatButtonCheck(){
  
  bool lastCheatToggle = cheatToggle;
  cheatToggle = digitalRead(_cheatPin);
  static bool key = 1;
...
void Chess::resetCheck(){
  digitalWrite(_columnZero, LOW);
  digitalWrite(_columnOne, LOW);
  digitalWrite(_columnTwo, LOW);
  digitalWrite(_columnThree, LOW);
  digitalWrite(_columnFour, LOW);  
  digitalWrite(_columnFive, LOW);
  digitalWrite(_columnSix, LOW);
  digitalWrite(_columnSeven, LOW);        
  digitalWrite(_rowZero, HIGH);
  digitalWrite(_rowOne, HIGH);
  digitalWrite(_rowTwo, HIGH);
  digitalWrite(_rowThree, HIGH);
  digitalWrite(_rowFour, HIGH);
  digitalWrite(_rowFive, HIGH);
  digitalWrite(_rowSix, HIGH);
  digitalWrite(_rowSeven, HIGH);   
}


void Chess::blankLights(){
  for(int i=0;i<_numLights;i++){
  // pixels.Color takes RGB values, from 0,0,0 up to 255,255,255
...
void Chess::chooseAnalog(int numAnalog) {
  switch (numAnalog) {
    case 0:
      digitalWrite(_columnZero, HIGH);
      pieceVoltage = analogRead(_testColumnZero);
      break;
    case 1:
      digitalWrite(_columnOne, HIGH);
      pieceVoltage = analogRead(_testColumnOne);
      break;
    case 2:
      digitalWrite(_columnTwo, HIGH);
      pieceVoltage = analogRead(_testColumnTwo);
      break;
    case 3:
      digitalWrite(_columnThree, HIGH);
      pieceVoltage = analogRead(_testColumnThree);
      break;
    case 4:
      digitalWrite(_columnFour, HIGH);
      pieceVoltage = analogRead(_testColumnFour);
      break; 
    case 5:
      digitalWrite(_columnFive, HIGH);
      pieceVoltage = analogRead(_testColumnFive);
      break;  
    case 6:
      digitalWrite(_columnSix, HIGH);
      pieceVoltage = analogRead(_testColumnSix);
      break;        
    case 7:
      digitalWrite(_columnSeven, HIGH);
      pieceVoltage = analogRead(_testColumnSeven);
      break;
  }
}
  
void Chess::matrixToArray(int i, int a) {
  // If i is even
  if (i % 2 == 0){
  ledNumber = (i*8) + a;
  }
...
bool Chess::PieceMovement(char colour, int piece, int row, int column){
  bool possible = 0; 
  int originalRow;
  int originalColumn; 
...
bool Chess::CheckFunction(char colour){      //W is white, B is black 
...
bool Chess::wPawnMoves() { // moves forward 1 if empty, Move diagonal if enemy, move 2 if start
...
bool Chess::bPawnMoves() { // moves forward 1 if empty, Move diagonal if enemy, move 2 if start Returns true if possible, Returns false if not (USED FOR CHECKMATE)
  bool possiblemoves = false;
...
bool Chess::wQueenMoves() {      /* Queen Code is Almagamation of the bishop and rook  */
    bool possiblemoves = 0; 
...
void Chess::checkColourButton() {

  lastColour1State = colour1State;
  colour1State = digitalRead(_colourPin1);
  ...
  colour2State = digitalRead(_colourPin2);
  ...
  colour3State = digitalRead(_colourPin3);
...
void Chess::colourswap1() { // colour of piece picked up
...
void Chess::colourswap2() { // colour of piece moves
...
void Chess::colourswap3() { // colour of piece captures
...
unsigned short Chess::myrand(void) {
  unsigned short r = (unsigned short)(seed % MYRAND_MAX);
  return r = ((r << 11) + (r << 7) + r) >> 1;
}

short Chess::D(short q, short l, short e, unsigned char E, unsigned char z, unsigned char n) {
  short m, v, i, P, V, s;
...
void Chess::chessMatrixConverter() {
